...
本项目不仅完整复原了所有题目与 27 种人格（含常规与隐藏人格），还独家接入了 **AI 大模型分析引擎**。在你完成基础测试后，可以一键生成千人千面、极度毒舌的专属性格辅助分析报告。

## ✨ 核心特性
- **📊 15 维度匹配雷达**：不玩二分法，用更复杂的行为切面来勾勒真实人设。
- **🎭 27 种完整人格图鉴**：包含极其稀有的“CTRL 拿捏者”以及隐藏彩蛋“DRUNK 酒鬼”、“HHHH 傻乐者”等。
- **💥 人格碰撞 (CP分析)**：支持双人人格对比，多维度解析火花冲突与互补。
- **🤖 AI 毒舌批斗**：传入个人维度得分，动态生成毫不留情但细思极恐的赛博诊断书。
- **⚡ 纯本地极速运行**：前端采用原生 HTML/CSS/Vanilla JS 打造，无重度框架，轻量级。
- **🔍 极致 SEO 优化**：全量 Schema.org 结构化数据接入，搜索引擎友好。
...
*(注：AI 功能需要在对应的 worker 云函数端配置 AI API Key)*
